...
def percTrain(X,t, maxIts,online):
    ...
    if online == False: #to change on the final algorithm
        n = np.size(X,0)
        m = np.size(X,1)
        ...
        return ( w, state) # we also return the state of the perceptron in case the algorithm ended cause the maximum of iterations was reached
...
percep, state =  percTrain(train_set_features, train_set_value,500,False)  
...
for k in range(0,1000):
    ...
    train_set_features_5d.append([1,x_1,x_2,x_1**2,x_2**2,x_1*x_2]) # features of each image : 1 x1 x2 x1² x2² x1 x2
    
train_set_features_5d = np.transpose(np.array(train_set_features_5d))# reshape to have a matrix where each colonn represent the 6_features of an image

percep_5d, state_5 = percTrain(train_set_features_5d, train_set_value,500,False)
...
train_set_images = np.transpose(np.array(train_set_images))# reshape to have a matrix where each colonn represent an image of the data_set

percep_full, state_full = percTrain(train_set_images, train_set_value,500,False)
