...
def main():
    ...
    nr_sample = 35
    digits = [0, 7]
    sigma_range = np.linspace(4, 50, 20)
    ...
    plt.title("Influence of support vectors")
    ...
    for i in enumerate(c_range):
        plt.plot(sigma_range, avg_test_error[(len(sigma_range) * i[0]):(len(sigma_range) * (i[0] + 1))],
                 label=f"c={i[1]}")
    plt.legend()
    ...
    for i in enumerate(c_range):
        plt.plot(sigma_range, avg_train_error[(len(sigma_range) * i[0]):(len(sigma_range) * (i[0] + 1))],
                 label=f"c={i[1]}")
    plt.legend()
    ...
    plt.figure()
    plt.contourf(sigma_range, c_range, np.reshape(avg_test_error, (len(c_range), len(sigma_range))))
    ...
    plt.figure()
    plt.contourf(sigma_range, c_range, np.reshape(avg_train_error, (len(c_range), len(sigma_range))))
